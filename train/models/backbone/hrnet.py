...
class PoseHighResolutionNet(nn.Module):
    ...
    def forward(self, x, out_high_res_feat=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        ...
        if out_high_res_feat:
            high_res_feat = x[0].clone()
...
def get_cfg_defaults(pretrained, width=32, downsample=False, use_conv=False, all_pretrained=False):
...
def hrnet_w48(
    ...
    if "bedlam" in pretrained_ckpt_path:
        all_pretrained = True
    else:
